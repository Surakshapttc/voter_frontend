      ...
      axios.post("http://localhost:3001/getUpdateData", {id})
              .then(
                  (response) => {
                   ...
                   setAdarCard(response.data.adarcardno);
            ...
            const handleSubmit = (e) =>{
                e.preventDefault();
                try {
                    axios.post("http://localhost:3001/Update", {reg_no,name,voterid_no,gender,date_birth,adress,imgurl,adarcardno})
                    .then ((response)=>{
  ...
  return (
    <div>
        ...
        <input placeholder='Enter vnr'  name='pnumber' type='number' value={reg_no} onChange={(e) => setRegNo(e.target.value)} required />
            ...
            <input placeholder='Enter km_without_dr'  name='km_without_dr' type='number' value={adress} onChange={(e) => setAdress(e.target.value)} required />
            ...
            <input placeholder='Enter km_without_dr'  name='km_without_dr' type='number' value={adarcardno} onChange={(e) => setAdarCard(e.target.value)} required />
